...
- [x] トップ画面（モード選択）
- [x] 出題画面: チャンピオン画像＋名前 → AP / AD の2ボタン → 即座に正誤表示 → 次へ
- [x] 10問1セット、最後にスコア表示
- [x] `HYBRID` は通常モードで出題しない
- [x] スマホ最適化（**利用者はほぼスマホで遊ぶ前提**。片手で押せるボタン配置にすること）

**完了条件**: スマホで10問通して遊べる → OK（Phase 2 入力途中でも、`answer` が入った
チャンピオンだけを出題プールにするので遊べる。出題プールが 10 体未満ならあるぶんだけ出題する）

実装メモ:

- 出題プールは `src/lib/data.ts` の `normalModeChampions()`（`excluded` でなく `answer` が AP/AD）。
- 出題順は乱数なので、`src/components/QuizClient.tsx` で `ssr: false` にしてクライアントでだけ描画する
  （静的 HTML に焼き込むと水和時に食い違う）。
- 結果画面（`QuizResult.tsx`）は間違えたチャンピオン一覧を出す（7章）。Phase 4 の `ap_ratio` 表示と
  シェアテキストはここに足す。
